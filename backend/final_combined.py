"""最终方案：rembg (深色主体) + R-G 通道 (淡色补充)。

rembg 擅长处理深色区域，R-G 通道能检测淡色墨水。
两者取 max 互补，覆盖整个印章。
"""
import sys, types
...
# 线性映射：R-G 值 → alpha
rg_enhanced = np.clip(roi_rg.astype(np.float32) * 4, 0, 255).astype(np.uint8)

# ====== 结合：取 max ======
combined = np.maximum(rembg_alpha.astype(np.uint16), rg_enhanced.astype(np.uint16))
...
cv2.imwrite("test_output/FINAL_combined.png", rgba)
print(f"\n输出: test_output/FINAL_combined.png")

# 统计
print(f"\n最终 alpha 统计:")
total = h * w
in_circle_total = inside_circle.sum()
print(f"  alpha>0: {(full_alpha>0).sum()} ({(full_alpha>0).sum()/total*100:.2f}%)")
print(f"  alpha>128: {(full_alpha>128).sum()} ({(full_alpha>128).sum()/total*100:.2f}%)")
print(f"  alpha>200: {(full_alpha>200).sum()} ({(full_alpha>200).sum()/total*100:.2f}%)")
print(f"  圆内 alpha>0: {(full_alpha[inside_circle]>0).sum()}/{in_circle_total} ({(full_alpha[inside_circle]>0).sum()/in_circle_total*100:.1f}%)")
